...
patch('BE/tauri.conf.json', /^(\s*"version":\s*")[^"]*(")/m, `$1${to}$2`)
...
patch('BE/Cargo.toml', /^version = "[^"]*"/m, `version = "${to}"`)

// Cargo.lock: solo la voce del pacchetto dashai. Il file e' a fine riga CRLF
// su Windows e LF sui runner, quindi il salto di riga va accettato in entrambe
// le forme.
patch('BE/Cargo.lock', /(name = "dashai"\r?\nversion = ")[^"]*(")/, `$1${to}$2`)
